...
if st.session_state.processed_df is not None:
    ...
    with st.sidebar:
        ...
        # FIX: Check whether Date column is empty before min/max
        if 'Date' not in df.columns or df['Date'].dropna().empty:
            st.error("The Date column is empty or contains no valid dates. Please check the date format in your uploaded file.")
            st.stop()
    ...
    if not filtered_df.empty:
        ...
        if 'Impressions' in filtered_df.columns:
            ...
            st.markdown(
                ...
                f"for **High Impressions** "
                f"(Total: {top_impression_type['Impressions']:,})"
            )
        else:
        ...
        st.markdown(
            ...
            f"for **High Total Sales** "
            f"(Total: {top_sales_value['Total Sales']:,.2f})"
        )
        ...
        if not roas_by_campaign.empty:
            ...
            st.markdown(
                f"**Focus on Campaign:** `{top_roas_campaign['Campaign Name']}` "
                f"for **High ROAS** ({top_roas_campaign['ROAS']:.2f})"
            )
        else:
        ...
        if not risky_campaigns.empty:
            st.markdown(
                "**Minimize or Review Campaigns with Low ROAS (<2) and High Spend:**"
            )

            for _, row in risky_campaigns.iterrows():
                st.markdown(
                    f"- `{row['Campaign Name']}` with ROAS: "
                    f"{row['ROAS']:.2f} and Budget Spent: "
                    f"{row['Estimated Budget Consumed']:,.2f}"
                )
        else:
            st.markdown(
        ...
        risky_targeting['ROAS'] = risky_targeting.apply(
            lambda x: x['Total Sales'] / x['Estimated Budget Consumed']
            if x['Estimated Budget Consumed'] > 0 else 0,
            axis=1
        )

        risky_targeting = risky_targeting[
            (risky_targeting['ROAS'] < 2) &
            (risky_targeting['Estimated Budget Consumed'] > 1000)
        ]

        if not risky_targeting.empty:
            st.markdown(
                "**Minimize Targeting Types with Low ROAS (<2) and High Spend:**"
            )

            for _, row in risky_targeting.iterrows():
                st.markdown(
                    f"- `{row['Targeting Type']}` with ROAS: "
                    f"{row['ROAS']:.2f} and Budget Spent: "
                    f"{row['Estimated Budget Consumed']:,.2f}"
                )
        else:
            st.markdown(
                "No targeting types identified as risky based on current filters."
            )
    ...
    if not filtered_df.empty:
        ...
        st.warning(
            "Filtered data is empty or missing required columns for ROI calculation."
        )
    ...
    if not filtered_df.empty:
        ...
        budget_campaign_df = budget_campaign_df.sort_values(
            by='Estimated Budget Consumed',
            ascending=False
        )
    ...
    roi_df = roi_df[
        roi_df['Estimated Budget Consumed'] > 0
    ]

    if not roi_df.empty:
        roi_df['ROI'] = (
            roi_df['Total Sales'] /
            roi_df['Estimated Budget Consumed']
        )
    ...
    sales_by_targeting = (
        filtered_df.groupby('Targeting Type')['Total Sales']
        .sum()
        .reset_index()
    )
    ...
    if not sales_by_targeting.empty:
        ...
        st.info(
            "No data available to show Sales by Targeting Type pie chart."
        )
    ...
    if not filtered_df.empty:
        ...
        filtered_df_copy['Budget Range'] = filtered_df_copy.apply(
            classify_budget,
            axis=1
        )
        ...
        filtered_df_copy['Total ROAS'] = filtered_df_copy.apply(
            lambda row:
                row['Total Sales'] / row['Estimated Budget Consumed']
        ...
        st.dataframe(
            display_budget_df.sort_values(by='Budget Range')
        )

        # Aggregate sales by Budget Range for plotting
        sales_by_budget_df = (
            display_budget_df.groupby('Budget Range')['Total Sales']
            .sum()
            .reset_index()
        )
        ...
        st.info(
            "No data available for Sales by Budget Range table."
        )
    ...
    if not filtered_df.empty:
        ...
        st.info(
            "No data available to show Top 5 Campaigns."
        )
    ...
    if not filtered_df.empty:
        ...
        roas_campaign_df = roas_campaign_df[
            roas_campaign_df['Estimated Budget Consumed'] > 0
        ]

        roas_campaign_df['ROAS'] = (
            roas_campaign_df['Total Sales'] /
            roas_campaign_df['Estimated Budget Consumed']
        )
        ...
        st.dataframe(
            top_roas_campaigns[['Campaign Name', 'ROAS']]
        )

    else:
        st.info(
            "No data available to show Top 5 Campaigns by ROAS."
        )
    ...
    if not filtered_df.empty:
        ...
        if not low_roas_campaigns.empty:
            ...
            st.info(
                "No campaigns with ROAS less than 2 found."
            )

    else:
        st.info(
            "No data available to evaluate ROAS."
        )